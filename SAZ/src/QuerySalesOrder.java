...
public class QuerySalesOrder extends JFrame {
    ...
    public QuerySalesOrder() {   //初始化窗体设置
    ...
    void addComponents() { //自定义方法，添加组件
        ...
        model.addColumn("订单编号"); //添加一列，列名为员工编号
        model.addColumn("订单日期");
        model.addColumn("产品编号");
        model.addColumn("单价");
        model.addColumn("数量");
        model.addColumn("订单总额");
        model.addColumn("销售人员");
        model.addColumn("销售区域");
        model.addColumn("支付方式");
        model.addColumn("出货仓库");
