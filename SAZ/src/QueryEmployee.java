...
public class QueryEmployee extends JFrame {
    ...
    void addComponents() { //自定义方法，添加组件
        ...
        model.addColumn("员工职位");
        model.addColumn("联系方式");
        model.addColumn("入职日期");
        model.addColumn("退休年份");
    ...
    void searchByEmpID(String empID, String name) {
        try {
            //sql字符串存储要执行的查询语句
            String sql = "select * from HUMANRESOURCES.EMPLOYEE where EmployeeID=\'" + empID + "\' or Name=\'" + name + "\'";
            ...
            while (rs.next()) { //next用于读取一行数据，若存在，返回true，否则返回false
                ...
                v.add(rs.getString(4)); //从结果集获取第四列数据
